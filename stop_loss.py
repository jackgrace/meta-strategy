...
- CC/SCALE/VALUE adsets (today's metrics):
    stop:    ACTIVE + spend>$600 & ROAS<1.6
    restart: PAUSED + spend>$600 & ROAS>=1.6
...
#   Pause:   spend > $600 & ROAS < 1.6
#   Restart: spend > $600 & ROAS >= 1.6
# Applies to campaigns whose name contains CC, SCALE, or VALUE.
CVS_ADSET_SPEND_THRESHOLD = 600.0
...
def run_stop_loss(config: Config, dry_run: bool = False) -> tuple[list[StopLossAction], list[AdsetAction]]:
    ...
    # CC/SCALE/VALUE ad-level intra-day stop-loss is OFF for now.
    # The 3d ad-level hard-kill (ad_kill_3d.py, daily) is the sole ad-level
    # lever for these campaigns.
    ad_stop = 0
    ad_restart = 0
    ad_fail = 0
    ...
    # Single rule:
    #   Pause:   ACTIVE + spend > $600 & ROAS < 1.6
    #   Restart: PAUSED + spend > $600 & ROAS >= 1.6
