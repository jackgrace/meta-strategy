...
- SCALE adsets (today's metrics):
    stop:    ACTIVE + spend>$2000 & ROAS<1.5
    restart: PAUSED + spend>$2000 & ROAS>=1.5 (intra-day if ROAS improves)
...
- CBO adsets (today's metrics):
    stop:    ACTIVE + spend>$2000 & ROAS<1.5
    restart: PAUSED + spend>$2000 & ROAS>1.5
...
#   Pause:   ACTIVE + spend > $2000 & ROAS < 1.5
#   Restart: PAUSED + spend > $2000 & ROAS >= 1.5
...
def run_stop_loss(config: Config, dry_run: bool = False) -> tuple[list[StopLossAction], list[AdsetAction]]:
    ...
    # Single rule:
    #   Pause:   ACTIVE + spend > $2500 & ROAS < 1.5
    ...
    for adset_id in cbo_adset_ids:
        ...
        # STOP-LOSS: ACTIVE + spend > $2000 + ROAS < 1.5
        if (status == "ACTIVE"
