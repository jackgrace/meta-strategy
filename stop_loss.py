...
TESTING_ADSET_CEILING_BUDGET_SHARE = 0.3
...
SCALE_CBO_AD_CHEAP_CPC_PROTECT = 1.0     # legacy, unused by primary rule
    ...
    while url:
        ...
        for row in data.get("data", []):
            ...
            for a in row.get("actions", []) or []:
                ...
                elif a.get("action_type") == "initiate_checkout":
                    checkouts = int(float(a.get("value", 0)))
...
def run_stop_loss(config: Config, dry_run: bool = False) -> tuple[list[StopLossAction], list[AdsetAction]]:
    ...
    for adset_id in testing_adset_ids:
        ...
        threshold = max(
            TESTING_ADSET_CEILING_MIN_SPEND,
            daily_budget * TESTING_ADSET_CEILING_BUDGET_SHARE,
        )

        atcs = data.get("atcs", 0)
        checkouts = data.get("checkouts", 0)
        cost_per_atc = data.get("cost_per_atc", 0)
        if atcs > 0:
            events, event_label = atcs, "ATC"
        else:
            events, event_label = checkouts, "checkout"
        cost_per_event = spend / events if events > 0 else 0
        protected = (
            atcs >= TESTING_ADSET_PROTECT_MIN_ATCS
            and cost_per_atc < TESTING_ADSET_PROTECT_CPA_ATC
        )
        early_pause = (
            not protected
    ...
    for ad_id, ad in today_ads.items():
        ...
        adset_gate_open = (
            as_spend > SCALE_CBO_AD_ADSET_SPEND_GATE
            and as_roas < SCALE_CBO_AD_ADSET_ROAS_GATE
        )
        ad_roas_recovered = roas >= SCALE_CBO_AD_ROAS_THRESHOLD
        expensive_atc = atcs > 0 and cost_per_atc > SCALE_CBO_AD_CPA_ATC_THRESHOLD

        branch_a_fires = (
            spend > SCALE_CBO_AD_SPEND_THRESHOLD
            and expensive_atc
        )
        branch_b_fires = spend > SCALE_CBO_AD_SPEND_THRESHOLD_GRACE

        # PAUSE — adset gate open, ad still underperforming, and either branch fires.
        if (status == "ACTIVE"
            and adset_gate_open
            and not ad_roas_recovered
            and (branch_a_fires or branch_b_fires)):
            reason_bits = [
                f"adset spend ${as_spend:.0f}>${int(SCALE_CBO_AD_ADSET_SPEND_GATE)} & adset ROAS {as_roas:.2f}<{SCALE_CBO_AD_ADSET_ROAS_GATE}",
                f"ad ROAS {roas:.2f}<{SCALE_CBO_AD_ROAS_THRESHOLD}",
            ]
            if branch_a_fires:
                reason_bits.append(
                    f"branch A: spend ${spend:.2f}>${int(SCALE_CBO_AD_SPEND_THRESHOLD)} & CPA/ATC ${cost_per_atc:.2f}>${int(SCALE_CBO_AD_CPA_ATC_THRESHOLD)}"
                )
            elif branch_b_fires:
                reason_bits.append(
                    f"branch B: spend ${spend:.2f}>${int(SCALE_CBO_AD_SPEND_THRESHOLD_GRACE)} (grace expired)"
                )
...
def build_stop_loss_slack_message(
    ...
    blocks.append({
        ...
        "text": {"type": "mrkdwn", "text": (
            ...
            f" | B) spend>\\${int(SCALE_CBO_AD_SPEND_THRESHOLD_GRACE)} (grace)_\n"
