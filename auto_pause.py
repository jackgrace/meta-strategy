...
def find_pause_candidates(
    ...
    for ad_id in all_ad_ids:
        ...
        # Only check SCALE or CBO campaigns (whole word).
        campaign_parts = [p.strip() for p in campaign_name.upper().replace("|", " ").split()]
        if not ({"SCALE", "CBO"} & set(campaign_parts)):
            skipped_campaign += 1
            continue
    ...
    logger.info(
        ...
        f"{skipped_campaign} not SCALE/CBO │ {skipped_off} already OFF │ "
...
def build_pause_slack_message(candidates: list[PauseCandidate], dry_run: bool) -> dict | None:
    ...
    blocks.append({
        ...
        "text": {"type": "mrkdwn", "text": (
            ...
            f"Rule: SCALE/CBO campaigns │ created >{MIN_AD_AGE_DAYS} days ago │ <${SPEND_THRESHOLD:.0f} spend in last {LOOKBACK_DAYS} days │ ad+adset name!contains RUN\n"
